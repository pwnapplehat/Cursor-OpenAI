...
| Multiple gateway requests per message in Cursor's dashboard | Normal and expected: Hermes executes tools locally, so each tool-loop step is its own API call → its own metered Cursor request. Native Cursor runs its loop inside one request; a bridged agent can't. | Nothing to fix - it's inherent to any agent driving a stateless OpenAI-style API. Session/agent reuse (already on) keeps token costs down via prompt caching. |
